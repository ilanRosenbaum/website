...
def extract_code_blocks(md_content, languages=["jsx", "javascript"]):
    ...
    if not code_blocks and (
        "return (" in md_content or "const " in md_content or "import " in md_content
    ):
...
def main():
    ...
    if len(sys.argv) != 3:
        ...
        print(
            "Example: python convert_md_to_react.py ./content/Post.md ./components/Post.tsx"
        )
        sys.exit(1)
    ...
    needs_wrapping = not contains_component_definition(combined_code) or re.search(
        r"^\s*return\s*\(", combined_code, re.MULTILINE
    )
    ...
    print(
        f"React component '{component_name}' successfully created at '{output_component_path}'."
    )
